def check_str(text: str) -> str: 
    """The function counts the number of lowercase 
    ...
    for i in text:
        if i.islower():
            counter += 1
    return f'There are {len(text) - counter} upper case and {counter} lower case'

print(check_str('ThiS is TEST texT for ThE tasK'))
...
def get_ranges(l:list) -> str: 
    """The function collapses the list. If the numbers are in order, then 
...
print(get_ranges([-6,-3,-2,-1,0,1,3,4,5,7,8,9,10,11,14,15,17,18,19,20]))