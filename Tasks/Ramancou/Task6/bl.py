    ...
    for num,name,art,price in query_result:
        ...
        if art not in art_counte:
            ...
            print(
                ...
                f"Название: {name}",
                f"Артикул: {articul}",
                f"Цена: {float(price)*count_items}",
                f"Количество: {count_items}",
                sep="\n"
            )
            print('--' * 10)
...
def get_cart_data():
    ...
    while True:
        ...
        try:
            ...
            check_cart_info()
            print(f'Shopping Cart Total Price is {get_price()} руб.\n')
            print("*" * 35)
            user_inp = int(
            ...
            if user_inp == 0:
                ...
                clear()
                print(f"DELETE cart page\n")

                
                check_cart_info()
                get_pretty()
                user_del = input(
                    "Input number of item which you want to delete\nType 'all' to delete all entries\nEnter to go back\nChoose to continue: ")
                clear()
                if user_del == 'all':
                    data.delete_all_items()
            ...
            clear()
            ent = input("Press enter to exit from your cart: ")
            break